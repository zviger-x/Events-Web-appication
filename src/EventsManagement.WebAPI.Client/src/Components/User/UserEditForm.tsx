...
export default function UserEditForm() {
    ...
    const validate = async (): Promise<boolean> => {
        ...
        if (user?.email) {
            const existingUsers = await APIConnector.GetUserByEmail(user.email);
            const isDuplicate = !!existingUsers && existingUsers.id !== user.id; // Exclude current user
            if (isDuplicate) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (await validate()) {
            if (user) {
                await APIConnector.EditUser(user)
                ...
                <div className="event-details">
                    <Form onSubmit={handleSubmit}>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
                            ...
                            <Form.Field style={{ flex: '1 1 45%' }}>
                                ...
                                <Form.Input
                                    type="datetime-local"
                                    name="birthDate"
                                    value={user.birthDate ? new Date(user.birthDate).toISOString().slice(0, 16) : ''}
                                    onChange={handleInputChange}
                        ...
                        <div style={{ paddingTop: '20px' }}>
                            <Button floated='right' type="submit" color="green">Update</Button>
